...
import { Send, Activity, MessageSquare, Stethoscope, Archive, Compass, GraduationCap, Shield, LogOut, ClipboardList, Loader2, Menu, X, Globe, User, LayoutGrid, Scale, Paperclip, Image as ImageIcon, ShieldAlert, Megaphone, ChevronRight } from 'lucide-react';
import { Language, ChatMessage, ScoredSyndrome, UserAccount, TcmDiagnosisResult, Promotion, AdminMessage } from './types';
...
import SuperSaintDashboard from './components/SuperSaintDashboard';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        let profile = await db.users.get(user.uid);
        if (!profile) {
          // Check if this is an anonymous user who just logged in as haxor
          const saved = localStorage.getItem('tcm_active_session');
          const isHaxor = saved && JSON.parse(saved).username === 'haxor';
          
          profile = {
            uid: user.uid,
            username: isHaxor ? 'haxor' : (user.displayName || user.email || 'User'),
            email: user.email || '',
            role: (user.email === 'developertarbiyahcare@gmail.com' || isHaxor) ? 'super_saint' : 'user',
            status: 'active',
            subscriptionExpiry: Date.now() + (30 * 24 * 60 * 60 * 1000), // 30 days default
            createdAt: Date.now()
          };
        ...
        await db.users.update(user.uid, { 
          lastLogin: Date.now(),
          activeSessions: (profile.activeSessions || 0) + 1 
        });
  ...
  const [activePanel, setActivePanel] = useState<'chat' | 'diagnosis' | 'wuxing' | 'ukom' | 'archive' | 'atlas' | 'invoice' | 'bmi' | 'super_saint'>('chat');
  ...
  useEffect(() => {
    if (currentUser?.uid) {
      const unsubMsgs = db.messages.getByUser(currentUser.uid, (msgs) => setAdminMessages(msgs));
      const unsubPromos = db.promotions.getAll((promos) => setPromotions(promos));
      return () => {
        unsubMsgs();
        unsubPromos();
      };
    ...
    <div className="flex h-[100dvh] bg-purple-50 text-purple-950 overflow-hidden font-sans">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-purple-100 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col h-[100dvh] md:h-full shadow-2xl md:shadow-none`}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto scrollbar-hide">
           ...
           {currentUser?.role === 'super_saint' && (
             <SidebarTab id="super_saint" label="Super Saint" icon={ShieldAlert} activeClass="bg-red-600 shadow-red-900/40" />
           )}
        </nav>
      ...
      <div className="flex-1 flex flex-col h-full bg-purple-50 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 scrollbar-hide bg-[radial-gradient(circle_at_top_right,rgba(168,85,247,0.05),transparent)]">
          {activePanel === 'super_saint' && currentUser?.role === 'super_saint' && (
            <SuperSaintDashboard />
          )}
          {activePanel === 'chat' && (
        ...
        {/* Right Sidebar: Promotions & Messages */}
        {activePanel !== 'super_saint' && (
          <aside className="hidden xl:flex w-80 flex-col border-l border-purple-100 bg-white/50 backdrop-blur-sm overflow-y-auto p-4 gap-6">
            {/* Admin Messages */}
              ...
              <section className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  {adminMessages.map(msg => (
                    <div key={msg.id} className={`p-3 rounded-2xl border ${msg.isRead ? 'bg-white/50 border-purple-100' : 'bg-purple-100 border-purple-200 shadow-sm'} transition-all`}>
                      <p className="text-sm text-purple-900">{msg.content}</p>
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-[10px] text-purple-400">{new Date(msg.timestamp).toLocaleDateString()}</span>
                        {!msg.isRead && (
                          <button 
                            onClick={() => msg.id && db.messages.markAsRead(msg.id)}
                            className="text-[10px] font-bold text-tcm-primary hover:underline"
                          >
                            Mark as read
                          </button>
                        )}
            ...
            <section className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-4">
                {promotions.length === 0 ? (
                  <div className="p-8 text-center border-2 border-dashed border-purple-100 rounded-3xl">
                    <p className="text-xs text-purple-300 italic">No active promotions</p>
                  </div>
                ) : (
                  promotions.map(promo => (
                    <div key={promo.id} className="group relative bg-white border border-purple-100 p-4 rounded-3xl shadow-sm hover:shadow-md transition-all overflow-hidden">
                      <div className="absolute top-0 left-0 w-1 h-full bg-tcm-primary opacity-0 group-hover:opacity-100 transition-all" />
                      <h4 className="text-sm font-bold text-purple-900 mb-1">{promo.title}</h4>
                      <p className="text-xs text-purple-600 line-clamp-3 mb-3">{promo.content}</p>
                      {promo.link && (
                        <a 
                          href={promo.link} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 text-[10px] font-bold text-tcm-primary hover:gap-2 transition-all uppercase tracking-widest"
                        >
