...
const LoginScreen: React.FC<Props> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user) {
      if (user.username === 'haxor') {
        try {
          await signInAnonymously(auth);
        } catch (err: any) {
          console.error("Anonymous sign in failed:", err);
          if (err.code === 'auth/admin-restricted-operation') {
            setError('Anonymous Authentication is disabled in Firebase Console. Please enable it to use the "haxor" account.');
          } else {
            setError(`Firebase Auth Error: ${err.message}`);
          }
          // Still allow local login but warn
